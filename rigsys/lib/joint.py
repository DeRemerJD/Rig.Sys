...
def aim(nodes=[], target=[], aimAxis="+x", upAxis="-z", upObj=None, 
    ...
    for x in nodes:
        ...
        #Aim!
        if upObj == "vector":
            ac=cmds.aimConstraint(target, nodes[x], aim=aimVector, wut=upType, u=upVector, wu=worldVector)
        else:
            ac=cmds.aimConstraint(target, nodes[x], aim=aimVector, wut=upType, wuo=upObj, u=upVector)
        cmds.delete(ac)
...
def aimSequence(targets=[], aimAxis="+x", upAxis="-z", upObj=None, 
    ...
    for i in range(0, len(targets)):
        ...
        #Otherwise, aim it properly.
            ...
            if upObj == "vector":
                ac=cmds.aimConstraint(targets[i+1],targets[i],aim=aimVector,wut=upType,u=upVector,wu=worldVector)
            else:
                ac=cmds.aimConstraint(targets[i+1],targets[i],aim=aimVector,wut=upType,wuo=upObj,u=upVector,wu=worldVector)
            cmds.delete(ac)
