...
class skinClusterImportExport(deformerBase.DeformerModuleBase):
    ...
    def __init__(
        ...
        isMuted: bool = False,
        mirror: bool = False,
        importSCLS: bool = False,
        exportSCLS: bool = False,
        createSCLS: bool = True,
        joints: list = [],
        object: str = "",
        path: str = "",
        maxInfluences: int = 4,
    ...
    def run(self) -> None:
        ...
        if len(self.joints) != 0:
            ...
            if len(gatherMissing) > 0:
                cmds.error(
                    f"Missing the following joints {gatherMissing}; for {self.obj}_scls"
                )
    ...
    def createSkinCluster(self):
        cmds.skinCluster(
            self.joints, self.obj, n=f"{self.obj}_scls", tsb=True, mi=self.maxInfluences
        )
