...
class FollicleEye(motionBase.MotionModuleBase):
    ...
    def buildModule(self) -> None:
        ...
        inLabel = f"{self.side}_{self.label}_{fullRangeUp[0]}"
        outLabel = f"{self.side}_{self.label}_{fullRangeUp[-1]}"
        iJnt = cmds.createNode("joint", n=inLabel)
        oJnt = cmds.createNode("joint", n=outLabel)
        cornerJoints.append(iJnt, oJnt)
        ...
        eyejoint = cmds.createNode("joint", n=f"{self.side}_{self.label}_Eye")
        cmds.xform(eyejoint, ws=True, t=cmds.xform(
                f"{self.side}_{self.label}_{self.proxies["Eyeball"].name}", q=True, ws=True, t=True
            ))
        '''
        ...
        for u, l in zip(upLidJoints, upLidJoints):
        ...
        # Make Main controls
        upLidGrp = cmds.createNode("transform", n=f"{self.side}_{self.label}_{self.proxies["_Up"].name}_Main_grp")
        loLidGrp = cmds.createNode("transform", n=f"{self.side}_{self.label}_{self.proxies["_Lo"].name}_Main_grp")
        upLidCtrl = cmds.createNode("transform", n=f"{self.side}_{self.label}_{self.proxies["_Up"].name}_Main_CTRL", p=upLidGrp)
        loLidCtrl = cmds.createNode("transform", n=f"{self.side}_{self.label}_{self.proxies["_Lo"].name}_Main_CTRL", p=loLidGrp)
        ...
        if buildFollicles:
            ...
            for uf, lf in zip(upFolOffsets, loFolOffsets):
                ...
                if useMesh:
                    ...
                    cmds.connectAttr(f"{uDM}.outTranslate", f"{ucpom}.inPosition")
                    ...
                    uDM = cmds.createNode("decomposeMatrix", n=f"{upGroups[index]}_DM")
                    cmds.connectAttr(f"{upGroups[index]}.worldMatrix[0]", f"{uDM}.inputMatrix")
                    cmds.connectAttr(f"{uDM}.outTranslate", f"{ucpos}.inPosition")
