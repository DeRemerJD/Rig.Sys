...
class Limb(motionBase.MotionModuleBase):
    ...
    def buildSkeleton(self):
        ...
        if self.poleVector is None:
            ...
            self.poleVector = cmds.rename(
                par, f"{self.side}_{self.label}_PoleVector")
        ...
        socketConnector = cmds.createNode(
            "joint", n=f"{baseJoints[1]}_Connection")
    ...
    def buildBaseControls(self, baseJoints, IKJoints, FKJoints, socketConnector):
        ...
        globalTransform = cmds.createNode(
            'transform', n=f"{baseJoints[1]}_Global")
        localTransform = cmds.createNode(
            'transform', n=f"{baseJoints[1]}_Local")
        ...
        oc = cmds.orientConstraint(
            [globalTransform, localTransform], fkPar, n=f"{fkPar}_Lock_oc", mo=0)[0]
        cmds.setAttr(f"{oc}.interpType", 2)
        pc = cmds.pointConstraint(
            localTransform, fkPar, n=f"{fkPar}_Lock_pc", mo=0)
        ...
        loT = cmds.createNode(
            'transform', n=f"{self.side}_{self.label}_loPV", p=ikCtrl)
        ...
        rev = cmds.createNode(
            "reverse", n=f"{self.side}_{self.label}_IKFK_rev")
        ...
        for ctrl in FKControls:
            cmds.addAttr(ctrl, ln="IK_FK_Switch",
                         proxy=f"{ikCtrl}.IK_FK_Switch", at="float", min=0, max=1, dv=0, k=1)
        cmds.addAttr(pvCtrl, ln="IK_FK_Switch",
                     proxy=f"{ikCtrl}.IK_FK_Switch", at="float", min=0, max=1, dv=0, k=1)
        ...
        pc = cmds.pointConstraint(
            baseJoints[2], midGrp, n=f"{midGrp}_pc", mo=0)
        oc = cmds.orientConstraint(
            [upRollJoints[1], loRollJoints[1]], midGrp, n=f"{midGrp}_oc", mo=0)[0]
        cmds.setAttr(f"{oc}.interpType", 2)
    ...
    def buildRibbon(self, baseJoints, upRollJoints, loRollJoints, midOffset, endOffset):
        ...
        pc = cmds.pointConstraint(
            [upRollJoints[0], midOffset], ribbonGroups[1], n=f"{ribbonGroups[1]}_pc", mo=0)[0]
        oc = cmds.orientConstraint(
            [upRollJoints[0], upRollJoints[1]], ribbonGroups[1], n=f"{ribbonGroups[1]}_oc", mo=0)[0]
        cmds.setAttr(f"{pc}.{upRollJoints[0]}W0", 2)
        cmds.setAttr(f"{oc}.interpType", 2)
        cmds.setAttr(f"{oc}.{upRollJoints[0]}W0", 2)
        pc = cmds.pointConstraint(
            [upRollJoints[0], midOffset], ribbonGroups[2], n=f"{ribbonGroups[2]}_pc", mo=0)[0]
        oc = cmds.orientConstraint(
            [upRollJoints[0], upRollJoints[1]], ribbonGroups[2], n=f"{ribbonGroups[2]}_oc", mo=0)[0]
        ...
        pc = cmds.pointConstraint(
            [endOffset, midOffset], ribbonGroups[4], n=f"{ribbonGroups[4]}_pc", mo=0)[0]
        oc = cmds.orientConstraint(
            [endOffset, loRollJoints[1]], ribbonGroups[4], n=f"{ribbonGroups[4]}_oc", mo=0)[0]
        cmds.setAttr(f"{pc}.{midOffset}W1", 2)
        cmds.setAttr(f"{oc}.interpType", 2)
        cmds.setAttr(f"{oc}.{loRollJoints[1]}W1", 2)
        pc = cmds.pointConstraint(
            [endOffset, midOffset], ribbonGroups[5], n=f"{ribbonGroups[5]}_pc", mo=0)[0]
        oc = cmds.orientConstraint(
            [endOffset, loRollJoints[1]], ribbonGroups[5], n=f"{ribbonGroups[5]}_oc", mo=0)[0]
