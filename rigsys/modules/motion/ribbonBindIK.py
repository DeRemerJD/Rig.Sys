...
class RibbonBindIK(motionBase.MotionModuleBase):
    ...
    def buildRibbonControls(self, ribbon, allFollicles):
        ...
        metaRibbon = cmds.duplicate(
            ribbon, n=f"{self.side}_{self.label}_Meta_rbn", rc=True
        )[0]
        ...
        for i in range(self.spans):
            ...
            cmds.connectAttr(f"{metaRibbon}.local", f"{folShape}.inputSurface", f=True)
            ...
            grp = cmds.createNode(
                "transform", n=f"{self.side}_{self.label}_{i}_Meta_grp", p=fol
            )
            ctrl = cmds.createNode(
                "transform", n=f"{self.side}_{self.label}_{i}_Meta_CTRL", p=grp
            )
            jnt = cmds.createNode(
                "joint", n=f"{self.side}_{self.label}_{i}_Meta", p=ctrl
            )
            ctrlObject = ctrlCrv.Ctrl(
                node=ctrl,
                shape="sphere",
                scale=[
                    self.ctrlScale[0] * 1.125,
                    self.ctrlScale[1] * 1.125,
                    self.ctrlScale[2] * 1.125,
                ],
                offset=[0, 0, 0],
                orient=[0, 0, 0],
            )
            ...
            cmds.xform(grp, ws=True, t=cmds.xform(fol, q=True, ws=True, t=True))
        ...
        mScls = cmds.skinCluster(
            mFolJoints, ribbon, n=f"{ribbon}_scls", sm=0, omi=True, mi=4, tsb=True
        )
        param = 0
