...
class MotionModuleParenting(UtilityModuleBase):
    ...
    def run(self) -> None:
        ...
        for module in motionModules:
            if not module.isRun:
                logger.error(
                    f"Module not run: {module.getFullName()}. Unable to perform parenting."
                )
                continue
            ...
            logger.info(
                f"Parenting {module.getFullName()} at {plugNode} to {module.parent} at {socketNode}"
            )
            # TODO: Jacob, do the actual parenting here
            # module.socketPlugParenting()
            if module.parent is not None:
                socket = cmds.getAttr(
                    f"{module.parent}_MODULE.{module.selectedSocket}", asString=True
                )
                ...
                ptc = cmds.parentConstraint(
                    socket, module.plugs[module.selectedPlug], mo=1
                )[0]
